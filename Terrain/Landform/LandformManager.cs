using System;
using System.Collections.Generic;
using System.Linq;
using Godot;

public class LandformManager : TerrainAspectManager<Landform>
{
    public static Landform Peak { get; private set; } = new Landform("Peak", .8f, 0f, Colors.White);
    public static Landform Mountain { get; private set; } = new Landform("Mountain", .6f, 0f, Colors.DarkSlateGray);
    public static Landform Hill { get; private set; } = new Landform("Hill", .4f, .3f, Colors.Brown);
    public static Landform Plain  { get; private set; } = new Landform("Plain", 0f, .1f, Colors.SaddleBrown);
    public static Landform Sea  { get; private set; } = new Landform("Sea", Mathf.Inf, 0f, Colors.Blue);
    public static Landform River { get; private set; } = new River();
    public static Landform Urban { get; private set; } = new Urban();
    public LandformManager()
        : base(Sea, Plain, new List<Landform>{Urban, River, Peak, Mountain, Hill, Sea, Plain})
    {
        
    }
    
    public Landform GetAtPoint(MapPolygon poly, Vector2 pRel, Data data)
    {
        var close = poly.Neighbors.Refs().OrderBy(n => (poly.GetOffsetTo(n, data) - pRel).Length());
        var first = close.ElementAt(0);
        var second = close.ElementAt(1);
        var score = poly.GetScore(first, second, pRel, data, p => p.Roughness);
        return ByPriority.First(lf => lf.MinRoughness <= score);
    }
}