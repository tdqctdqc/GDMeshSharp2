...
public class RegimeItemStockDataTooltipTemplate : DataTooltipTemplate<RegimeItemDisplay>
{
    ...
    protected override List<Func<RegimeItemDisplay, Data, Control>> _fastGetters { get; }
        = new List<Func<RegimeItemDisplay, Data, Control>>
        {
            ...
            (t, d) => NodeExt.CreateLabel("Prod: " + t.Regime?.History.ProdHistory[t.Item.Name].GetLatest()),
            (t, d) => NodeExt.CreateLabel("Consumed: " + t.Regime?.History.ConsumptionHistory[t.Item.Name].GetLatest()),
            (t, d) => NodeExt.CreateLabel("Demanded: " + t.Regime?.History.DemandHistory[t.Item.Name].GetLatest()),
        };
