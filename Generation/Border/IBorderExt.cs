...
{
    public static TSeg First<TSeg, TPrim, TRegion>(this IBorder<TSeg, TPrim, TRegion> border)
    {
        return border.Elements[0];
    }
    public static TSeg Last<TSeg, TPrim, TRegion>(this IBorder<TSeg, TPrim, TRegion> border)
    {
    ...
    public static List<TEdge> GetOrderedEdges<TEdge, TPrimitive, TRegion>
        (this IReadOnlyGraph<TRegion, TEdge> graph, IEnumerable<TRegion> elements)
        where TEdge : IBorder<TEdge, TPrimitive, TRegion>
    {
        var nativeHash = elements.ToHashSet().ReadOnly();
        var borderNodes = graph.GetBorderElements(nativeHash);
