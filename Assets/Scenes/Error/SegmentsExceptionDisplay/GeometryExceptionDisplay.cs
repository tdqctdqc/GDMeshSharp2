...
public class GeometryExceptionDisplay : Node2D
{
    private int _iter = 0;
    ...
    public void Setup(GeometryException e)
    {
        ...
        message.RectPosition = -_buffer * 2f;
        AddChild(message);
    ...
    private void AddLabel(string text, Color col)
    {
        ...
        AddChild(label);
        label.RectPosition = _pos;
        label.RectPosition -= _buffer / 2f;
    }
    ...
    private ScrollContainer MakeScroll(IEnumerable<string> texts, Vector2 size, float width)
    {
        ...
        scroll.RectPosition = _pos + width * Vector2.Right;
        AddChild(scroll);
        return scroll;
    }
